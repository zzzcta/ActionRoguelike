...
void USAction_ProjectileAttack::StartAction_Implementation(AActor* Instigator)
{
	...
	if (Character)
	{
		...
		UGameplayStatics::SpawnEmitterAttached(MuzzleFlash, Character->GetMesh(), FName("Muzzle_01"),
		                                       FVector::ZeroVector,
...
void USAction_ProjectileAttack::AttackDelay_Elapsed(ACharacter* InstigatorCharacter)
{
	if (ensure(InstigatorCharacter))
	{
		...
		if (GetWorld()->SweepSingleByObjectType(OutHit, TraceStart, TraceEnd, FQuat::Identity, ObjectQueryParams,
